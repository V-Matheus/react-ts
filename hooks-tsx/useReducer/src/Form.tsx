...
type Action = { type: 'setNome', payload: string } | { type: 'setEmail', payload: string }
...
const Form = () => {
  ...
  return (
    <div>
      <Input label={`Nome: ${state.nome}`} value={state.nome} onChange={({target}) => dispatch({type: 'setNome', payload: target.value})}/>
